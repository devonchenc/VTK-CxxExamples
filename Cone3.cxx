...
int main(int argc, char *argv[])
{
    ...
    vtkRendererSource *rs = vtkRendererSource::New();
    rs->SetInput(ren1);
    rs->WholeWindowOn();
    vtkTIFFWriter *w = vtkTIFFWriter::New();
    w->SetInputConnection(rs->GetOutputPort());

    ren1->GetActiveCamera()->Azimuth(90);
    renWin->Render();
    rs->Modified();
    w->SetFileName("cone3_1.tif");
    w->Write();

    ren1->GetActiveCamera()->Azimuth(45);
    ren2->GetActiveCamera()->Azimuth(45);
    renWin->Render();
    rs->Modified();
    w->SetFileName("cone3_2.tif");
    w->Write();
